...
public:
    enum SPOT_STATE {
        EMPTY = 0,                  //empty means the board spot is not occupied
        BLACK = 1,
        WHITE = 2
    };
    static const int SIZE = 15;
    std::array<std::array<int, SIZE>, SIZE> board;      //use an array to memory board
    int empty_count;                //to count how many spots not been occupied
    int cur_player;                 //to memorize who is playing(white or black)
    bool done;                      
    int winner;
private:
    ...
    bool is_spot_on_board(Point p) const {      //use is_spot_on_board to check whether the spot is in the region
        return 0 <= p.x && p.x < SIZE && 0 <= p.y && p.y < SIZE;
    }
    int get_disc(Point p) const {               //check where the spot is and the disc there
        return board[p.x][p.y];
    }
    void set_disc(Point p, int disc) {          //give disc to the spot
        board[p.x][p.y] = disc;                 //所以其實它是在整個board上確認每個spot的disc
    }
    bool is_disc_at(Point p, int disc) const {  //idk
    ...
    bool is_spot_valid(Point center) const {    //idk
...
public:
    ...
    bool put_disc(Point p) {                    
        if(!is_spot_valid(p)) {                 //is the spot is not vaild
            winner = get_next_player(cur_player);
            done = true;
            return false;
        }
        set_disc(p, cur_player);               //otherwise, set the disc for the spot for cur_player
        empty_count--;                         //we check a point, so the empty_count should minus one
        // Check Win
        if (checkwin(cur_player)) {             //check if the cur_player win the game
            done = true;
            winner = cur_player;
        }
        if (empty_count == 0) {                 //平手
    ...
    std::string encode_spot(int x, int y) {             //draw the board    
