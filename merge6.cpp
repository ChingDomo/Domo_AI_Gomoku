...
void State::set_state_value(){
    int temp_value = 0;
    // int line5, line5opo, live4, live4opo, rush4, rush4opo;
    // int live3, live3opo, sleep3, sleep3opo, live2, live2opo, sleep2, sleep2opo;
    for(int p=0; p<SIZE;p++){
        for(int q=0; q<SIZE; q++){
            if(p<SIZE-4 || q<SIZE-4 || q>=4){   
                // line5 += linefive(this->board, p, q, this->player);
                // line5opo += linefive(this->board, p, q, 3-this->player);
                temp_value += 10000000*linefive(this->board, p, q, this->player);
                temp_value -= 20000000*linefive(this->board, p, q, 3-this->player);
            }
            if(p<SIZE-5 || q<SIZE-5 || q>=5){
                // live4 += livefour(this->board, p, q, this->player);
                // live4opo += livefour(this->board, p, q, 3-this->player);
                temp_value += 20000*livefour(this->board, p, q, this->player);
                temp_value -= 22000*livefour(this->board, p, q, 3-this->player);
            }
            if(p<SIZE-5 || p<SIZE-4 || q<SIZE-5 || q<SIZE-4 || q>=5 || q>=4){
                // rush4 += rushfour(this->board, p, q, this->player);
                // rush4opo += rushfour(this->board, p, q, 3-this->player);
                temp_value += 14000*rushfour(this->board, p, q, this->player);
                temp_value -= 16000*rushfour(this->board, p, q, 3-this->player);
            }
            if(p<SIZE-4 || p<SIZE-5 || q<SIZE-4 || q<SIZE-5 || q>=4 || q>=5){
                // live3 += livethree(this->board, p, q, this->player);
                // live3opo += livethree(this->board, p, q, 3-this->player);
                temp_value += 13000*livethree(this->board, p, q, this->player);
                temp_value -= 15000*livethree(this->board, p, q, 3-this->player);
            }
            if(p<SIZE-4 || p<SIZE-5 || p<SIZE-6 || q<SIZE-4 || q<SIZE-5 || q<SIZE-6 || q>=4 || q>=5 || q>=6){
                // sleep3 += sleepthree(this->board, p, q, this->player);
                // sleep3opo += sleepthree(this->board, p, q, 3-this->player);
                temp_value += 10000*sleepthree(this->board, p, q, this->player);
                temp_value -= 12000*sleepthree(this->board, p, q, 3-this->player);
            }
        }
    }
    //cout << "line 5 | line5opo | live4 | live4opo | rush4 | rush4opo | live3 |  live3opo | sleep3 | sleep3opo | live2 | live2opo | sleep2 | sleep2opo";
    //cout << line5 << " ; " << line5opo << " ; "  << live4 << " ; " << live4opo << " ; " << rush4 << " ; " << rush4opo << " ; " << live3 << " ; " << live3opo << " ; " << sleep3 << " ; " << sleep3opo << " ; " << live2 << " ; " << live2opo << " ; " << sleep2 << " ; " << sleep2opo << "\n";
...
Point implement_move(State* state, int depth){
    Point best_move;
    int compared_score = -INT_MAX;

    for(Point it: state->how_to_move){
        int score = -minimax(state->nextState(it), depth-1, false);
        cout << "score: " << score << "\n" << "x y" << it.x << " " << it.y << "\n";
...
void write_valid_spot(std::ofstream& fout) {
    ...
    // while(true) {
        ...
        depth = 1;
        auto move = implement_move(&root, depth);
